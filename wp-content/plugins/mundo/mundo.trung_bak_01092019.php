...
?>
    ...
    <form method="post" class="form-search-travel-style" >
        <div id="travel_style_search" class="et_pb_search et_pb_module et_pb_bg_layout_light et_pb_text_align_left  et_pb_search_0">
            ...
            <input type="submit" name="submit" value="<?php echo __('Search', 'mundo');?>" class="make-enquire" id="submit_travel_style" data-id ="tours_travel_style_honeymoon" />
        </div>
...
//                    }
        ...
        <div id="travel_style_search" class="et_pb_search et_pb_module et_pb_bg_layout_light et_pb_text_align_left  et_pb_search_0">
            <label> <?php echo __('Search tour', 'mundo');?>:</label>
            <input type="text" name="tour_name" id="tour_name" class="et_pb_s" placeholder="<?php echo __('Enter tour name', 'mundo');?>"/>
            <input type="submit" name="submit" value="<?php echo __('Search', 'mundo');?>" class="make-enquire" id="submit_travel_style" data-id ="tours_travel_style"/>
        </div>
    </form>
<?php
    $html = ob_get_clean();
...
?>
    ...
    ?>
    <form method="post" id="form-search-home" action="<?php echo $link;?>">
        <input type="hidden" name="action" value="form_search_home" />
        <?php wp_nonce_field('form_search_home'); ?>
        <?php 
            $terms_destination = get_terms( array(
...
add_filter('custom_post_show_content', function($post_content, $module_id, $post) {
    if($module_id == 'city_of_vietnam_list_hotel'){
        ...
        $post_content .= '<div class=""><b class="text_gray">From:  </b>'.$discount_price_html.' <b class="'.$discount.' '.$text_gray.' price">$'.$price.' </b> <b class=" '.$discount.' text_gray "> per room</b></div></div>';     
        return $post_content;
    }
...
add_filter( 'custom-post-owlcarosel-content', function($custom_content,$module_id ){
    ...
    if($module_id=='post_hotel_detination'){
        ...
        $discount_price_html = '';
        $price = get_post_meta($post_id, 'price_from',true);
        $custom_content .= '<div class=""><b class="text_gray">From:  </b>'.$discount_price_html.' <b class="price'.$discount.' '.$text_gray.'">$'.$price.' </b> <b class=" '.$discount.' text_gray "> per room</b></div>';
                   ...
                   //print_r($btn_hide); exit;
                    $form .=  '<form method="GET" action="'.$url.'" name="frm_sic_tour" id="form_sic_tour" data-post="'.$post_id.'">    
                                        ...
                                        <div class="row  value '.$row_light.'" >
                                            ...
                                            <div class="col-md-3 col-sm-3 col-xs-4 sic-body-mobile departing '.$dem.' " id="departing" data-date="'.$departing['date']['timestamp'].'">'.$date_from.'</div>
                                            <div class="col-md-3 col-sm-3 col-xs-4 sic-body-mobile show_on_mobile" id="finishing">'.$finishing.'</div>
                                            <div class="col-md-2 col-sm-2 total col-xs-4 sic-body-mobile show_on_mobile '.$dem.'" id="total"> $'.$total.'</div>

                                            

                                            <div class="col-md-3 col-sm-3 show_on_desktop" id="finishing">'.$finishing.'</div>
...
function get_post_hotel_html($post,$module_id = ''){
  ...
  //  $html .= apply_filters('custom_post_show_content', $html, $module_id, $post);
    ...
    $lang = pll_current_language();
    $price = mundo_exchange_rate($price, $lang);
    $html .= '<div class=""><b class="text_gray">From:  </b>'.$discount_price_html.' <b class="'.$discount.' '.$text_gray.' price">$'.$price.' </b> <b class=" '.$discount.' text_gray "> per room</b></div></div>';     
    $html .= '      </div>';
    $html .= '</article>
            </div>';
    return $html;        
...
function mundo_get_price_last_6_month($post_id){
    ...
    $prices_lang = !empty($prices_lang)?:0;
