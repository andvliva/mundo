...
class Slider_Slider_Item_3_Image extends ET_Builder_Module {
	function init() {
		$this->name                        = esc_html__( 'Slide 3 img', 'liva-divi' );
		$this->slug                        = 'et_pb_slide_3_img';
		...
		$this->advanced_setting_title_text = esc_html__( 'New Slide', 'liva-divi' );
		$this->settings_text               = esc_html__( 'Slide Settings', 'liva-divi' );
	...
	function get_fields() {
		$fields = array(
			...
			'__video_embed' => array(
				'type' => 'computed',
				'computed_callback' => array( 'Slider_Item_3_img', 'get_video_embed' ),
	...
	function maybe_inherit_values() {
		...
		if ( ! empty( $et_pb_slider_3_img ) && ! is_admin() && ! et_fb_is_enabled() ) {
			foreach ( $et_pb_slider_3_img as $slider_attr => $slider_attr_value ) {
				// Get default value
...
//        d($this->shortcode_atts);
       ...
       // d($parallax_image_background);
		...
		$image = '' !== $image
			? sprintf( '<div data-p="137.50"><img data-u="image" src="%1$s" alt="%2$s" /></div>',
				esc_url( $image ),
				esc_attr( $image_alt )
			)
			: '';
...
//		d($arr_content);
       ...
       // d($slide_content); // noi dung text  aaa
//        d($button);
//		d($style);
//		d($image); 
//		d(esc_attr( $class ));
//		d($video_background);
//		d($parallax_image_background);
//		d($data_dot_nav_custom_color);
//		d($data_arrows_custom_color);
...
new Slider_Slider_Item_3_Image;