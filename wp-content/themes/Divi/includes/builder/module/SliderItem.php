...
class ET_Builder_Module_Slider_Item extends ET_Builder_Module {
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		if (is_page('experiences') && get_query_var('travel_style')) {
			$term 			= get_term_by('slug', get_query_var('travel_style'), 'travel_style');
			$term_data 		= get_term_meta($term->term_id, 'cc_bg', true);
			$url_cc			= wp_get_attachment_image_src($term_data, 'full');
			$cc_image 		= (!empty($url_cc)) ? $url_cc[0] : '';
			$style			.= ($cc_image != '') ? 'style="background-image: url(' . $cc_image . ')"' : '';
		} else {
			$cc_image = '';
			$cc_class = '';
		}
