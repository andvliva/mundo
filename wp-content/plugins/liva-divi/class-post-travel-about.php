<?php
class Travel_About_Post extends ET_Builder_Module_Blog {
     function init() {
    	parent::init();
        $this->name = __( 'Travel about post', 'liva-divi' );
        $this->slug = 'et_pb_travel_about';
...
// 		$this->global_settings_slug = 'et_pd_custom_post_slider';
    ...
    function shortcode_callback( $atts, $content = null, $function_name ) {
        ...
        $more_text           = apply_filters('travel_about_more_text', $more_text, $module_id);
		...
		if ( have_posts() ) {
			...
			<?php
								...
								<?php
                                    ...
                                    if($filter_thumb != null) {
                                    	...
                                    	if ($i==1) {
								            ...
								            if($thumbnail_id) {
								                $src = mundo_get_attachment_image_src($thumbnail_id, 'know_more_first');
                                    		 ...
                                    		 $thumbnail_id = get_post_thumbnail_id($post);
									            if($thumbnail_id) {
									                $src = mundo_get_attachment_image_src($thumbnail_id, 'know_more_all');
									                $image = sprintf('<img src="%s" alt="%s" />', $src, $post->post_title);
									                echo $image;
									            }
                                    	}
                ...
                <?php
                    ...
                    $class_i = $i==1?:'all';
                ?>
            <div class="wrapper-content <?php echo 'postition-content-'.$class_i;?>">
