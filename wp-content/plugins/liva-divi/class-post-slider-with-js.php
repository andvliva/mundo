<?php
class Post_Slider_With_Js extends Post_Owlcarousel {
    function init() {
    	parent::init();
        $this->name = __( 'Post Slider With Js', 'liva-divi' );
        $this->slug = 'et_pb_post_slider_with_js';
    ...
    function shortcode_callback( $atts, $content = null, $function_name ) {
        ...
        switch ($current_lang) {
                case 'en':
                    ...
                    $link_travel_style = $module_id=='departure_month'?$link_travel_style.'/all':$link_travel_style;
                    $link_travel_style = $module_id=='travel_guide_tour'?$link_travel_style.'/all':$link_travel_style;
                    break;
                case 'es':
        ...
        $tab_header = apply_filters('post_slider_with_js_tab_header', $tab_header, $module_id);
		...
		if ( have_posts() ) {
			...
			while ( have_posts() ) {
				...
				?>
			<article data='test' id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class  ); ?>>
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) { ?>
				...
				<?php
                    ...
                    $departure_dates = get_post_meta($post->ID, 'departure_date',true);
                    $departure_date_text = '';
                    $i = 1;
                        ...
                        //      echo '<div class="departure_date content"><b> Departures date: <span class="departure_date">'.$departure_date_text.'</span></b> </div>'; 
                        // }
                    ...
                    //     $route = (get_post_meta($post->ID,'route',true))?:'';
                    //     echo '<div class="route content"><b>'.$route.'</b> </div>';
                    // }
                    $highlight_city = get_post_meta($post->ID, 'highlight_city', true);
                    if($highlight_city){
                        echo '<div class="highlight_city content"><b>'.__("Highlights",'liva-divi').' : </b> <b class="text_gray">'.$highlight_city.'</b> </div></div> <hr>';
                    }
                    ...
                    if ( 'on' !== $show_content ) {
                        ...
                        $discount_price_html = '';
                        if($discount_price){
                            $discount_price_html = '<b class="discount_price"> '.$symbol_exchange.$discount_price.'</b><span class="discount_price"> pp </span>';
                            $text_gray = 'text_gray';
                            $discount = 'discount';
                        }
