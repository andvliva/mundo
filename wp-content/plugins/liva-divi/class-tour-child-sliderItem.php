...
class Slider_Slider_Tour_Item extends Slider_Tours {
	function init() {
	    parent::init();
		$this->name                        = esc_html__( 'Slider For Tour Child', 'liva-divi' );
		$this->slug                        = 'et_pb_slider_tours_child';
    ...
    function shortcode_callback( $atts, $content = null, $function_name ) {
        ...
        $more_text           = __('View tour', 'liva-divi');
        $more_text           = apply_filters('custom_post_more_text', $more_text, $module_id);
        $type_tour        = $this->shortcode_atts['type_tour'];
        if ( empty($type_tour)) {
        	$type_tour='travel_style';
        }
		global $paged;
...
//        print_r($taxonomies); exit;
       // $output = '<div class = "travel_style">';
//        foreach($taxonomies as $taxonomy){
//           $output .= '<span>'.$taxonomy->name.' | </span>'; 
//        }
//        $output .= '</div>';
		//$output .= $this->show_tours($fullwidth,$show_thumbnail, $args);
       // print_r($args); exit;
        //return $output;
        query_posts( $args );
        if ( have_posts() ) {
				...
				if ( ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) {
					...
					elseif ( '' !== $thumb && 'on' === $show_thumbnail ) :
						if ( 'on' !== $fullwidth ) {
							...
							$travel_style_arr = wp_get_post_terms( $post->ID, 'travel_style');
							$travel_style = $travel_style_arr[0]->name;
							if ($travel_style) {
								echo '<p class="tour-travel-style">'.$travel_style.'</p>';
							}
						}
						?>
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) { ?>
				...
				<?php 
					if($module_id=='post_tour'){
						...
						$duration_nights = get_post_meta($post->ID,'duration_nights',true);
						$duration_nights = $duration_nights?$duration_nights:'';
						echo $duration_days.__( ' Days', 'liva-divi' ).' / '.$duration_nights.__( ' Nights', 'liva-divi' );
						$price = get_post_meta($post->ID,'sale',true);
						$icon_sale = $price?'icon_sale':'';
						$price = get_post_meta($post->ID,'new_tour',true);
						$icon_new_tour = $price?'icon_new_tour':'';
						echo $icon_sale;
						echo $icon_new_tour;
					}
				?>
					<<?php echo $processed_header_level; ?> class="entry-title 2"><a href="<?php esc_url( the_permalink() ); ?>"><?php the_title(); ?></a></<?php echo $processed_header_level; ?>>
				<?php } ?>
				...
				<?php
                    ...
                    $show_content = apply_filters('custom_post_show_content', $show_content, $module_id ,$type_tour);
                    d($show_content);
					if ( 'on' === $show_content ) {
						global $more;
					...
					if($module_id=='post_tour'){
						...
						echo __('From: ').$price.'<span> pp </span>';
					}
					...
					<article id="" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class ) ?>>
						...
						<?php if ( 'off' === $args['fullwidth'] || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
							<?php if ( ! in_array( $post_format, array( 'link', 'audio' ) ) ) { ?>
								<<?php echo $processed_header_level; ?> class="entry-title 3"><a href="<?php esc_url( the_permalink() ); ?>"><?php the_title(); ?></a></<?php echo $processed_header_level; ?>>
							<?php } ?>
...
//
//		$image = '' !== $image
//			? sprintf( '<div data-p="170.00"><img data-u="image" src="%1$s" alt="%2$s" /></div>',
//				esc_url( $image ),
//				esc_attr( $image_alt )
//			)
//			: '';
...
//				%4$s
//				<div class="et_pb_slide_description" style= " position: absolute;bottom: 1px;right: 12px;left: 0px;/* width: 68px; */height: 16px;color: red;/* top: 0px; */">
//					%1$s
