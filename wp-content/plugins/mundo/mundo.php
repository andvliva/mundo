        ...
        foreach($terms_combo as $key => $value)
      {
        ...
        switch ($current_lang) {
            case 'en':
				...
				$link_base 	= get_permalink(get_page_by_path('tours')) . 'filter/type_of_tour/';
				$link_end	= '/travel_style/tour_guide/duration/price_range/per_page/sort_by/search/departure';
				...
				if (!empty($arr)) {
					foreach ($arr as $k) {
						...
						$str .= sanitize_title($k);

						$i++;
					}
				}

				$link_travel = $link_base . $str . $link_end;
            ...
            case 'es':
				...
				$link_base 	= get_permalink(get_page_by_path('tours-es')) . 'filter/type_of_tour/';
				$link_end	= '/travel_style/tour_guide/duration/price_range/per_page/sort_by/search/departure';
            ...
            case 'pt':
                ...
                //$link_travel = get_permalink(get_page_by_path('experiences')).'?destination_combo="'.$value->slug.'"';
				$link_base 	= get_permalink(get_page_by_path('tours-pt')) . 'filter/type_of_tour/';
				$link_end	= '/travel_style/tour_guide/duration/price_range/per_page/sort_by/search/departure';
...
add_shortcode('form_search_tour_in_travel_style',function(){
    ...
    $travel_style = $_GET['travel_style'];
    $travel_style_queryvar =  get_query_var('travel_style_cat');
    if ($travel_style_queryvar) {
        $travel_style = array($travel_style_queryvar);
    }
    //print_r($travel_style);exit();
    $date_tour = $_GET['date_tour'];
    $terms = get_terms( 'duration', array(
        'hide_empty' => false,
    ) );
...
?>
    ...
    <form method="post" id="form-search-home" action="<?php echo $link; ?>" class="form_in_make_enquiry">
        ...
        <div class="wrapper-date_tour form-make-enquiry-duration">  
            <select name="date_tour" class="travel_style_search duration_search" multiple="true" search="true">
...
?>
    <?php 
        ...
        switch ($current_lang) {
            case 'en':
                $link = get_permalink(get_page_by_path('tours')) . 'filter/type_of_tour/category_destination/travel_style/tour_guide/duration/price_range/per_page/sort_by/search/departure';
                break;
            case 'es':
                $link = get_permalink(get_page_by_path('tours-es')) . 'filter/type_of_tour/category_destination/travel_style/tour_guide/duration/price_range/per_page/sort_by/search/departure';
                break;
            case 'pt':
                $link = get_permalink(get_page_by_path('tours-pt')) . 'filter/type_of_tour/category_destination/travel_style/tour_guide/duration/price_range/per_page/sort_by/search/departure';
                break;
            default:
...
//     exit();
// }
add_shortcode('list_save_tour', function(){
    ob_start();
    if (isset($_COOKIE['post_tour']))
    { 
            $cookie_tour =  $_COOKIE['post_tour'];
            ...
            $args = array(
              'post_type'   => 'tour',
              'post__in'    => $all_id_tour,
              'orderby' => 'post__in'
            );
            $favorate_tours = get_posts( $args );
            echo '<div class="wrapper-shorstlist">';
            foreach ($favorate_tours as $key => $value) {
                $src_image = get_post_thumbnail_id($value);
                $url_image = mundo_get_attachment_image_src($src_image,'favorate_tour');
                $price = mundo_get_price_last_6_month($value->ID);
                if($price){
                        ...
                        <div class="row">
                            <div class="col-md-7">
                                <a href="<?php echo get_permalink($value->ID) ;?>">
                                    <div class="col-md-5 img-shorlist">
                                        <img src="<?php echo $url_image;?>">
                                    </div>
                                    <div class="content-text-list col-md-7">
                                        <?php echo '<p class="title-shortlist">'.$value->post_title.'</p>' ;?>
                                        <?php $days = 0;
                                            $itinerarys = (get_post_meta($value->ID, 'itinerary',true))?:array(); 
                                            if($itinerarys){
                                                $days = count($itinerarys);
                                                echo '<p class="duration-shortlist">';
                                                echo  ($days==1)?$days.' '.__("Day",'mundo').' / 0 '.__("Night",'mundo').'': $days. ' '.__("Days",'mundo').' / '.($days-1).' '.__("Nights",'mundo').'';
                                                echo '</p>';
                                            }?>
                                    </div>
                                </a>
                            </div>
                            <div class="col-md-5 shortlisted-right">
                                <?php echo '<span>'.__('From','mundo');?> : <?php echo '<b>'.$symbol_exchange.$price.'</b> '.__('pp','mundo').'</span>';?>
                                    ...
                                    if($tour_type == 'customize'){
                                        ...
                                        switch ($current_lang) {
                                                case 'en':
                                                    $link_customize = get_permalink(get_page_by_path('booking-customize-tour')).'?post_id='.$post_id.'&month='.$min_date.'&adult=2&language='.$tour_lang_main.'&flight=no&book=BOOK+NOW';
                                                        $link_sic = get_permalink(get_page_by_path('booking-sic-tour')).'?post_id='.$post_id.'&date_from='.$time_sic.'&book=BOOK+NOW';;
                                                    $link = $type == 'customize'?$link_customize:$link_sic;
                                                    break;
                                                case 'es':
                                                    $link_customize = get_permalink(get_page_by_path('reservar-ahora')).'?post_id='.$post_id.'&month='.$min_date.'&adult=2&language='.$tour_lang_main.'&flight=no&book=BOOK+NOW';
                                                    $link_sic = get_permalink(get_page_by_path('reserva-de-salidas-garantizadas')).'?post_id='.$post->ID.'&date_from='.$time_sic.'&book=BOOK+NOW';;
                                                    $link = $type == 'customize'?$link_customize:$link_sic;
                                                    break;
                                                case 'pt':
                                                    $link_customize = get_permalink(get_page_by_path('reserva-personalizar')).'?post_id='.$post_id.'&month='.$min_date.'&adult=2&language='.$tour_lang_main.'&flight=no&book=BOOK+NOW';
                                                    $link_sic = get_permalink(get_page_by_path('reserva-saidas-regulares')).'?post_id='.$post_id.'&date_from='.$time_sic.'&book=BOOK+NOW';;
                                                    $link = $type == 'customize'?$link_customize:$link_sic;
                                                    break;
                                                default:
                                                    $link_customize = get_permalink(get_page_by_path('booking-customize-tour')).'?post_id='.$post_id.'&month='.$min_date.'&adult=2&language='.$tour_lang_main.'&flight=no&book=BOOK+NOW';
                                    ...
                                    <span class="black-color" data-post="<?php echo $post_id;?>"><i class="fa fa-heart"></i> <?php echo __('Shortlisted','mundo');?> </span> 
                            </div>
        ...
        echo '<h2 class="no-tour-found">'.__('No tour founed','mundo').'</h2>';
    }
    $html = ob_get_clean();
    return $html;
});
...
?>
    <form method="post" id="form-search-hotel" class="et_pb_row row" action="<?php echo admin_url('admin-post.php')?>">
        <?php 
            $terms_destination_par = get_terms( array(
                'taxonomy' => 'category-destination',
                'hide_empty' => false,
                //'child_of' => true,
                //'childless' => true,
            ...
            $terms_destination_par_id = wp_list_pluck($terms_destination_par, 'term_id');

            $terms_destination_all = get_terms( array(
                'taxonomy' => 'category-destination',
                'hide_empty' => false,
                //'child_of' => true,
                //'childless' => true,
                //'parent' => $terms_destination_par_id,
            ) );
            // $terms_destination = array_filter($terms_destination,function($my_term){
            //     return in_array($my_term->parent, $terms_destination_par_id);
            // });
            foreach ($terms_destination_all as $key_des => $value_des) {
                if ( in_array($value_des->parent, $terms_destination_par_id) ) {
                     $terms_destination[] = $terms_destination_all[$key_des];
                 } 
            }
        ...
        <div class="col-md-4">
            <p><?php echo __("Search Hotel",'mundo');?></p>
             <select  class="hotel-name-search icon" search="true" name="search_hotel" id="search_hotel" readonly>
                <option><?php echo __('Any hotel', 'mundo');?></option>
                <?php
                    $posts = get_posts(array(
                        'post_type' => 'hotel',
                        'posts_per_page' => -1,
        ...
        <div class="col-md-4">
            ...
            <select  class="hotel-city-search" search="true" name="hotel_city_search" id="hotel_city_search" readonly>
                <option><?php echo __('Any city', 'mundo');?></option>
                <?php
                    foreach($terms_destination as $term) {
                        if ($term->slug==$_GET['city']) {
                            printf('<option value="%s" selected>%s</option>', $term->slug, $term->name);
                        }else{
                            printf('<option value="%s">%s</option>', $term->slug, $term->name);
                        }
        ...
        <div class="col-md-4">
            ...
            <select class="hotel-style-search" search="true" name="hotel_style_search" id="hotel_style_search" readonly>
                <option> <?php echo __('Any style', 'mundo');?></option>
                <?php
                    foreach($terms_style as $term) {
                        printf('<option value="%s">%s</option>', $term->slug, $term->name);
                    }
...
?>
                        <div class='social_icons social_icons_blog '>
                                ...
                                <a href="<?php echo ot_get_option( 'facebook');?>" class="icon" target="_blank">
                               ...
                               <!--  <span aria-hidden="true" class="social-googleplus-ft">
                                    ...
                                    <a href="<?php echo ot_get_option( 'twitter');?>" class="icon" target="_blank">
                                        <span aria-hidden="true" class="social_twitter"></span>
                                    </a>
                                
                                
                                    <a href="<?php echo ot_get_option( 'instar');?>" class="icon" target="_blank">
                                        <span aria-hidden="true" class="social_instagram"></span>
                                    </a>
                                
                                
                                    <a href="<?php echo ot_get_option( 'linkedin');?>" class="icon" target="_blank">
                                        <span aria-hidden="true" class="social_linkedin_square"></span>
                                    </a>
                                
                                
                                    <a href="<?php echo ot_get_option( 'youtube');?>" class="icon" target="_blank">
...
add_filter('custom_post_show_content', function($post_content, $module_id, $post) {
    if($module_id == 'city_of_vietnam_list_hotel'){
        ...
        $post_content .= ' <span class="style">'.$hotel_styles[0]->name.'</span>  ';
        $discount_price_html = '';
        ...
        //$post_content .= '<hr> <div class=""><b class="text_gray">'.__("From",'Mundo').':  </b>'. $location[0]->name.'</b> </div></div>';      
        return $post_content;
    }
    ...
    if($module_id == 'expert_restaurant'){
...
add_filter('custom_posts_show_more', function($posts, $module_id, $args=array(), $offset, $fullwidth) {
    if(($module_id == 'tours_travel_style')||($module_id == 'tours_travel_style_honeymoon')){
        ...
        foreach ($posts_first as $key_post => $value_post) {
            ...
            $term_parent_other = array_filter($term_parent_other,function($item_parent){
                $terms = get_terms(array(
                    'taxonomy' => 'category-destination',
                    'hide_empty' => false,
                    'term_taxonomy_id' => $item_parent,
                ));
                if ($terms[0]->parent) {
                    return false;
                }else{
            ...
            //$quanty_all = get_post_meta($value_post->ID,'sort_tour_by_cat',true);
...
//        );
    ...
    if( $module_id == 'expert_restaurant' ) {
        $query_args['post_type']= 'expert'; 
        $query_args['meta_query']['expert_view_restaurant'] = array(
            'key' => 'expert_view_restaurant',
            'value' => 1,
        );
    }
...
add_filter('post_owlcarousel_posts', function($posts, $module_id) {
    if($module_id == 'thing_to_do_hotel'){
        ...
        if($post->post_type == 'hotel'){
            ...
            //     $content .= '</div>';
            // }
            foreach ($excursion_posts as $key_ex => $value_ex) {
                $expost = get_post($value_ex);
                //print_r($expost);exit();
                //echo 'test0';
                $content .= get_post_excursion_html($expost,'','0',' ');
                //echo 'test1';
                //exit();
            }
        } 
...
add_filter ('post_slider_with_js_tab_header', function($tab_header, $module_id){
    ...
    if($module_id == 'tour_in_list_restaurant'){
       // print_r('aa'); exit;
        return '';
    }
    if($module_id == 'expert_about_page'){
...
add_filter ('post_slider_with_js_query_args', function($query_args, $module_id){
     ...
     switch ($module_id) {
        ...
        case 'tour_in_list_hotel':
            ...
            return $query_args;
            break;
        case 'tour_in_list_restaurant':
            $query_args['post_type']= 'tour';
            $query_args['posts_per_page'] = 3;
            $query_args['meta_query'] = array(
                array(
                    'key' => 'show_list_restaurant',
                    'value' => 1,
                    'compare' => '=',
                ),
             );
        ...
        case 'post_tour_slider_js':
            ...
            if ($slug) {
                ...
                $query_args['meta_query']['select_type'] = array(
                        'key' => 'select_type',
                        'compare' => '=',
                        'value'  => 'customize',
                    );
...
function mundo_form_search_hotel(){
   $title= ($_POST['title'] == __('Any hotel','mundo'))?'':$_POST['title'];
    $city = $_POST['city'];
   //print_r($city); exit;
    ...
    if($title){
     ...
     if($city && $city!=__('Any city','mundo')){
        $args['tax_query']['destination'] = array(
            array(
                'taxonomy' => 'category-destination',
                'terms' => $city,
    ...
    if($hotel_style && $hotel_style != __('Any style','mundo') ){
...
function mundo_search_tour_in_travel_style(){
    ...
    if($duration){
        $args['tax_query']['duration_tour'] = array(
            array(
                'taxonomy' => 'duration_tour',
                'terms' => $duration,
                'field' => 'slug'
        ...
        // foreach ($duration as $key_du => $value_du) {
        //     $query_duration_tour[] = array(
        //         'taxonomy' => 'duration_tour',
        //         'terms' => $value_du,
        //         'field' => 'slug'
        //     );
        // }
        // $args['tax_query']['duration_tour'] = $query_duration_tour;
    }  
    //print_r($args);
    ...
    // print_r($posts);exit();
    if (!$order_price) {//ko sort theo giá thì sort theo term
        usort($posts, function($a, $b)
        {
            return strcmp($a->quanty_term, $b->quanty_term);
        });
...
function mundo_get_customize_price(){
    ...
    $price_sub = $symbol_exchange=='$'?'':'('.$symbol_exchange.$price_per_person.')';
    $total_sub = $symbol_exchange=='$'?'':'('.$symbol_exchange.$total_price.')';
   echo json_encode(array(
        'error' => 0,
        'mess' => '',
        'price_per_person' => '$'.get_price_usd($price_per_person).$price_sub,
        'total' => '$'.get_price_usd($total_price).$total_sub,
    ));
    exit();
}
    ...
    foreach ($tour_info[$tab] as $key => $value){
            switch ($key){ 
                case 'inclussion_detail':
                     $tab_content .= $value?'<div class="tab_content"><div class="content-inclussion-detail">'.str_replace("<!--more-->","</div><div class='content-inclussion-detail'>",apply_filters('the_content',$value)).'</div></div>':' ';
                    break;
    ...
    if ($data_fullwidth == 'off') {
        ...
        foreach($posts as $columns){
            ...
            foreach ($columns as $post){
                ...
                elseif($module_id=='city_of_vietnam_list_restaurants'){
                    $content  .= get_post_res_html($post, $module_id,$i, $data_fullwidth);$i++;
                }
                else{
...
function get_post_hotel_html($post,$module_id = ''){
  ...
  //  $html .= apply_filters('custom_post_show_content', $html, $module_id, $post);
    ...
    $html .= '<div class="content">';
    $html .= '<span class="location">'.$location[0]->name.'</span>   ';
    $html .= ' <span class="style">'.$hotel_styles[0]->name.'</span>  ';
    $discount_price_html = '';
    $price = get_post_meta($post->ID,'price_from',true);
    $symbol_exchange = '$';
    if (isset($_COOKIE['exchange_rate'])){
    ...
    $lang = pll_current_language();
    $price = mundo_exchange_rate($price, $lang);
    //$html .= '<div class=""><b class="text_gray">From:  </b>'.$discount_price_html.' <b class="'.$discount.' '.$text_gray.' price">'.$symbol_exchange.$price.' </b> <b class=" '.$discount.' text_gray "> per room</b></div>';     
    $html .= '      </div></div>';
    $html .= '</article>
          ';
    return $html;        
...
if (!function_exists('cc_tour_page_ajax_filter')) {
	function cc_tour_page_ajax_filter() {
		...
		$remove_action			= $_REQUEST['remove_action'];

		$departure_month		= $_REQUEST['departure_month'];
		$departure_start_date	= $_REQUEST['departure_start_date'];
		$departure_end_date		= $_REQUEST['departure_end_date'];

		$urlPathParts 			= explode('filter/', ltrim(parse_url($c_url,  PHP_URL_PATH), '/'));
		$queries				= explode('/', $urlPathParts[1]);

		//type of tour
		if (isset($type_of_tour)) {
			$old_type_of_tour = $queries[0];
			if ($checked == 1) {
				if ($old_type_of_tour == 'type_of_tour') {
					$new_url = str_replace('type_of_tour', $type_of_tour, $c_url);
				} else {
					$new_url = str_replace($old_type_of_tour, $old_type_of_tour . '+' . $type_of_tour, $c_url);
				}
			} else if ($checked == 0) {
				if ($old_type_of_tour == $type_of_tour) {
					$new_url = str_replace($old_type_of_tour, 'type_of_tour', $c_url);
				} else {
					$new_url 	= str_replace('/' . $type_of_tour . '+', '/', $c_url);
					$new_url	= str_replace('+' . $type_of_tour, '', $new_url);
				}
			}
		}
		...
		if (isset($category_destination)) {
			$old_category_destination = $queries[1];

			if ($checked == 1) {
				if ($old_category_destination == 'category_destination') {
					$new_url = str_replace('category_destination', $category_destination, $c_url);
				} else {
					$new_url = str_replace($old_category_destination, $old_category_destination . '+' . $category_destination, $c_url);
				}
			} else if ($checked == 0) {
				if ($old_category_destination == $category_destination) {
					$new_url = str_replace($old_category_destination, 'category_destination', $c_url);
				} else {
					$new_url 	= str_replace('/' . $category_destination . '+', '/', $c_url);
					$new_url	= str_replace('+' . $category_destination, '', $new_url);
				}
			}
		}
		...
		if (isset($travel_style)) {
			$old_travel_style = $queries[2];

			if ($checked == 1) {
				if ($old_travel_style == 'travel_style') {
					$new_url = str_replace('travel_style', $travel_style, $c_url);
				} else {
					$new_url = str_replace($old_travel_style, $old_travel_style . '+' . $travel_style, $c_url);
				}
			} else if ($checked == 0) {
				if ($old_travel_style == $travel_style) {
					$new_url = str_replace($old_travel_style, 'travel_style', $c_url);
				} else {
					$new_url 	= str_replace('/' . $travel_style . '+', '/', $c_url);
					$new_url	= str_replace('+' . $travel_style, '', $new_url);
				}
			}
		}
		...
		//tour_guide
		if (isset($tour_guide)) {
			$old_tour_guide = $queries[3];

			if ($checked == 1) {
				if ($old_tour_guide == 'tour_guide') {
					$new_url = str_replace('tour_guide', $tour_guide, $c_url);
				} else {
					$new_url = str_replace($old_tour_guide, $old_tour_guide . '+' . $tour_guide, $c_url);
				}
			} else if ($checked == 0) {
				if ($old_tour_guide == $tour_guide) {
					$new_url = str_replace($old_tour_guide, 'tour_guide', $c_url);
				} else {
					$new_url 	= str_replace('/' . $tour_guide . '+', '/', $c_url);
					$new_url	= str_replace('+' . $tour_guide, '', $new_url);
				}
			}
		}
		...
		if (isset($duration_tour)) {
			$old_duration_tour 	= $queries[4];

			$duration_tour 		= str_replace(',', '-', $duration_tour);
			$new_duration_tour 	= $duration_tour . esc_html__('day', 'mundo');
			$new_url 			= str_replace($old_duration_tour, $new_duration_tour, $c_url);
		}

		//filter price
		if (isset($filter_price)) {
			$old_filter_price = $queries[5];

			$filter_price 		= str_replace(',', '-', $filter_price);
			$new_filter_price 	= $filter_price . esc_html__('$', 'mundo');
			$new_url 			= str_replace($old_filter_price, $new_filter_price, $c_url);
		}

		//post per page
		if (isset($post_per_page)) {
			$old_post_per_page 	= $queries[6];
			$new_url			= str_replace($old_post_per_page, $post_per_page, $c_url);
		}

		//sort by
		if (isset($sort_by)) {
			$old_sort_by 	= $queries[7];
			$new_url 		= str_replace($old_sort_by, $sort_by, $c_url);
		}

		//seach tour
		if (isset($s_tour)) {
			$old_s_tour		= $queries[8];
			$new_url		= str_replace($old_s_tour, $s_tour, $c_url);
		}

		//departure
		if (isset($departure_month)) {
			$old_departure 	= $queries[9];
			$new_url 		= str_replace($old_departure, $departure_month, $c_url);
		}

		if (isset($departure_start_date) && isset($departure_end_date)) {
			$old_departure 	= $queries[9];
			$new_url 		= str_replace($old_departure, $departure_start_date . '+' . $departure_end_date, $c_url);
		}

		//remove action
		if (isset($remove_action)) {
			if ($remove_action == 'price') {
				$new_url = str_replace($queries[5], 'price_range', $c_url);
			}

			if ($remove_action == 'duration') {
				$new_url = str_replace($queries[4], 'duration', $c_url);
			}

			if ($remove_action == 'departure') {
				$new_url = str_replace($queries[9], 'duration', $c_url);
			}
		}
		...
		//build html
		$newurlPathParts 	= explode('filter/', ltrim(parse_url($new_url,  PHP_URL_PATH), '/'));
		$new_queries		= explode('/', $newurlPathParts[1]);

		$params					= array();

		if (isset($new_queries[0]) && $new_queries[0] != 'type_of_tour') {
			$params['type_of_tour'] = $new_queries[0];
		}

		if (isset($new_queries[1]) && $new_queries[1] != 'category_destination') {
			$params['category_destination'] = $new_queries[1];
		}

		if (isset($new_queries[2]) && $new_queries[2] != 'travel_style') {
			$params['travel_style'] = $new_queries[2];
		}

		if (isset($new_queries[3]) && $new_queries[3] != 'tour_guide') {
			$params['tour_guide'] = $new_queries[3];
		}

		if (isset($new_queries[4]) && $new_queries[4] != 'duration') {
			$params['duration_tour'] = $new_queries[4];
		}

		if (isset($new_queries[5]) && $new_queries[5] != 'price_range') {
			$params['filter_price'] = str_replace('$', '', $new_queries[5]);
		}

		if (isset($new_queries[6]) && $new_queries[6] != 'per_page') {
			$params['posts_per_page'] = $new_queries[6];
		}

		if (isset($new_queries[7]) && $new_queries[7] != 'sort_by') {
			$params['sort_by'] = $new_queries[7];
		}

		if (isset($new_queries[8]) && $new_queries[8] != 'search') {
			$params['s_tour'] = $new_queries[8];
		}

		if (isset($new_queries[9]) && $new_queries[9] != 'departure') {
			$params['departure'] = $new_queries[9];
		}
		...
		$request_arr = array(
			'text'		=> implode('', $html),
			'count'		=> $cc_tour_query->found_posts,
			'new_url'	=> $new_url,
...
if (!function_exists('cc_tour_build_query')) {
	function cc_tour_build_query($params) {
		...
		if ($category_destination) {
			...
			*/
			$cd_arr = explode('+', $category_destination);
		...
		if ($duration_tour) {
			$duration_tour = str_replace('day', '', $duration_tour);
			$z = explode('-', $duration_tour);
			$open_date 	= $z[0];
			$close_date	= $z[1];

			$default_open_date 	= array('1', '3', '6', '9', '12', '15', '18', '21', '24', '27', '30');
			$default_close_date = array('2', '5', '8', '11', '14', '17', '20', '23', '26', '29', '32');

			$arr = array();

			for ($i = 0; $i < 11; $i++) {
				for ($j = 0; $j < 11; $j++) {
					if ($open_date >= $default_open_date[$i] && $close_date <= $default_close_date[$j]) {
						for ($t = 0; $t < $j; $t++) {
							$arr[] = $default_open_date[$t] . '-' . $default_close_date[$t] . '-days';
						}
...
if (!function_exists('cc_custom_tour_page_data_content')) {
	function cc_custom_tour_page_data_content($posts, $max, $c_url) {
		...
		if (!empty($posts)) {
			...
			$html = esc_html__('Sorry, No result founds.', 'mundo');
			$html .= '<br>';
			$html .= esc_html__('Please change your filter or contact our travel consultant.', 'mundo');
		}
...
if (!function_exists('cc_tour_filter_custom_rewrite')) {
	function cc_tour_filter_custom_rewrite() {
		add_rewrite_tag('%type_of_tour%', '([^/]*)');
    	add_rewrite_tag('%category_destination%', '([^/]*)');
    	add_rewrite_tag('%travel_style%', '([^/]*)');
    	add_rewrite_tag('%tour_guide%', '([^/]*)');
    	add_rewrite_tag('%duration%', '([^/]*)');
    	add_rewrite_tag('%price_range%', '([^/]*)');
    	add_rewrite_tag('%per_page%', '([^/]*)');
    	add_rewrite_tag('%sort_by%', '([^/]*)');
    	add_rewrite_tag('%search%', '([^/]*)');
		add_rewrite_tag('%departure%', '([^/]*)');

    	    	add_rewrite_rule(
    		'^tours/filter/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/?$',
    		'index.php?page_id=25025&type_of_tour=$matches[1]&category_destination=$matches[2]&travel_style=$matches[3]&tour_guide=$matches[4]&duration=$matches[5]&price_range=$matches[6]&per_page=$matches[7]&sort_by=$matches[8]&search=$matches[9]&departure=$matches[10]',
    		'top'
    	);

    	add_rewrite_rule(
    		'^es/tours-es/filter/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/?$',
    		'index.php?page_id=25163&lang=es&type_of_tour=$matches[1]&category_destination=$matches[2]&travel_style=$matches[3]&tour_guide=$matches[4]&duration=$matches[5]&price_range=$matches[6]&per_page=$matches[7]&sort_by=$matches[8]&search=$matches[9]&departure=$matches[10]',
    		'top'
    	);

    	add_rewrite_rule(
    		'^pt/tours-pt/filter/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/([^/]*)/?$',
    		'index.php?page_id=25166&lang=pt&type_of_tour=$matches[1]&category_destination=$matches[2]&travel_style=$matches[3]&tour_guide=$matches[4]&duration=$matches[5]&price_range=$matches[6]&per_page=$matches[7]&sort_by=$matches[8]&search=$matches[9]&departure=$matches[10]',
    		'top'
    	);
		
    	add_rewrite_rule(
    		'^experiences/([^/]*)/?$',
    		'index.php?page_id=77&travel_style=$matches[1]',
    		'top'
    	);	
		
		add_rewrite_rule(
    		'^es/experiencias-2/([^/]*)/?$',
    		'index.php?page_id=6528&lang=es&travel_style=$matches[1]',
    		'top'
    	);	
		
		add_rewrite_rule(
    		'^pt/experiencias/([^/]*)/?$',
    		'index.php?page_id=16067&lang=pt&travel_style=$matches[1]',
    		'top'
    	);	
	...
	add_action('init', 'cc_tour_filter_custom_rewrite');
}
